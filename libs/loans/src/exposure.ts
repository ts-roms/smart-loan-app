...
export interface ExposureLoanInput {
  ...
  /**
   * What was actually expensed to Bad Debt, on a written-off loan.
   *
   * NOT the same as `principal`, and the difference is the whole point:
   * a borrower who repaid half before defaulting had half the contracted
   * amount written off, not all of it. Using `principal` would overstate
   * every partly-repaid write-off — safe in direction for a lending
   * decision, and simply wrong as a figure.
   *
   * Null on loans that were never written off, and on legacy rows where
   * the amount was not recorded; `consolidatedExposure` falls back to
   * `principal` there and there only.
   */
...
export interface ExposureLoan {
   ...
   * Carried on the row so the panel can show it per loan rather than
   * only as a lump in `excluded`.
   */
  writtenOff: number;
}
...
export interface ExposureExcluded {
  ...
  /**
   * What the lender actually expensed to Bad Debt — the balance at
   * write-off, not the contracted principal. See
   * `ExposureLoanInput.writeOffAmount`.
   */
...
export function loanExposure(input: ExposureLoanInput): ExposureLoan {
  ...
  return {
    ...
    /*
     * The recorded write-off amount when there is one, falling back to
     * the contracted principal only when it is absent. The fallback is
     * for legacy rows written off before the amount was captured — it
     * overstates a partly-repaid loan, which is the safe direction to be
     * wrong in, but it is a fallback and not the intent.
     */
    writtenOff:
      input.status === "WRITTEN_OFF"
        ? round2(
            input.writeOffAmount != null
...
export function consolidatedExposure(
  ...
  for (const row of rows) {
    ...
    if (row.status === "WRITTEN_OFF") {
      ...
      excluded.writtenOffPrincipal += row.writtenOff;
    }
  }
