...
import { AccountingRepository } from '../src/repositories/accounting.repository.js';
import { LoanProductRepository } from '../src/repositories/loan-product.repository.js';
import { DecisionRuleRepository } from '../src/repositories/decision-rule.repository.js';
import {
...
async function main() {
  ...
  console.log(`Chart of accounts: ${chart.created} created, ${chart.existing} already present.`);
  console.log(`Loan products:     ${products.created} created, ${products.existing} already present.`);
  console.log(`Decision rules:    ${rules.created} created, ${rules.existing} already present.`);
  console.log(`Permissions:       ${perms.created} created, ${perms.existing} already present.`);
  console.log(`Roles:             ${roleResult.created} created, ${roleResult.existing} reconciled.`);
  console.log(`Role assignments:  ${backfill.assigned} backfilled from User.role.`);
}
