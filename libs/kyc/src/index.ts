...
export function requiredDocsFor(extraRequired: KycDocumentType[] = []): KycDocumentType[] {
...
export function validateKyc(
  ...
  if (submissions.length === 0) {
    return { complete: false, status: 'NONE', missing: [...required], rejected: [] };
  }
  ...
  for (const s of submissions) if (!byType.has(s.documentType)) byType.set(s.documentType, s);

  const rejected = required.filter((t) => byType.get(t)?.status === 'REJECTED');
  const missing = required.filter((t) => !byType.has(t));
  const allVerified = required.every((t) => byType.get(t)?.status === 'VERIFIED');

  if (rejected.length > 0) return { complete: false, status: 'REJECTED', missing, rejected };
  if (allVerified) return { complete: true, status: 'VERIFIED', missing: [], rejected: [] };
