...
export async function scoringRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { customerId: string } }>(
    ...
    async (req, reply) => {
      ...
      if (!s) return reply.code(404).send({ error: 'NotFound', message: 'No score yet' });
      return s;
    },
  );
  ...
  app.get<{ Querystring: { score: string } }>('/tier', async (req, reply) => {
    ...
    if (!Number.isFinite(n)) {
      return reply.code(400).send({ error: 'BadRequest', message: 'score query required' });
    }
    return { score: n, tier: toTier(n) };
  });
}
