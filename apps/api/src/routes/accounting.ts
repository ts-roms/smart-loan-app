...
export async function accountingRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { id: string } }>('/journal/:id', async (req, reply) => {
    const e = await accounting.findEntryById(req.params.id);
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const entry = await accounting.postEntry(validated, { postedById: req.user.sub });
  ...
  app.get<{ Querystring: { from?: string; to?: string } }>(
    ...
    async (req, reply) => {
      ...
      if (Number.isNaN(from.getTime())) {
        return reply.code(400).send({ error: 'BadRequest', message: 'Invalid from date' });
      }
  ...
  app.post<{ Params: { year: string; month: string } }>(
    ...
    async (req, reply) => {
      ...
      if (!Number.isInteger(year) || !Number.isInteger(month) || month < 1 || month > 12) {
        return reply.code(400).send({ error: 'BadRequest', message: 'Invalid year/month' });
      }
  ...
  app.post<{ Body?: { year?: number; month?: number } }>(
    ...
    async (req, reply) => {
      ...
      try {
        return await accounting.accrueMonthlyInterest({ year, month }, req.user.sub);
      } catch (err) {
