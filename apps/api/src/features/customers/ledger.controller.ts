...
export class CustomerLedgerController {
  json = async (req: LedgerReq, reply: FastifyReply) => {
    ...
    const result = await req.customerServices!.ledger.build(req.params.id, {
      from,
      to,
      scope,
    });
  ...
  pdf = async (req: LedgerReq, reply: FastifyReply) => {
    ...
    const buf = await req.customerServices!.ledger.renderPdf(result.ledger);
  ...
  email = async (
    ...
    const result = await req.customerServices!.ledger.dispatchStatementReady(
      req.params.id,
    );
    if (!result) return reply.code(404).send({ error: "NotFound" });
