...
export const factorCreateSchema = z.object({
   ...
   *
   * Plain `.positive()`. It briefly had to be a `.refine` to work around
   * the OpenAPI 3.0 boolean `exclusiveMinimum`, which AJV rejects while
   * building the route — the server would not start. That is fixed in
   * `lib/openapi.ts` now, centrally, so the next schema does not have to
   * rediscover it.
   */
