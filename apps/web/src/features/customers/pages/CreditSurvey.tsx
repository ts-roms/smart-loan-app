...
import type { CreditTier, SurveyAnswer, SurveyQuestion } from '@loan/shared-types';
import {
  Button,
  Card,
  CardContent,
...
export function CreditSurveyPage() {
  ...
  const onSubmit = async () => {
    try {
      ...
      setResult({ score: res.score, tier: res.tier });
      toast.success(`Score ${res.score} · Tier ${res.tier}`);
    } catch (err) {
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Credit-scoring survey · {customer.data?.firstName} {customer.data?.lastName}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <p className="text-sm text-white/55">
          Answer all questions honestly. The score combines your responses
          with any prior loan history we have on file to produce a single
          credit grade between 300 and 850.
        </p>

        {result ? (
          <div className="rounded-md border border-white/10 bg-white/[0.04] p-6 text-center space-y-2">
            <div className="text-xs uppercase tracking-wider text-white/55">Final score</div>
            <div className="text-5xl font-semibold tracking-tight">{result.score}</div>
            <div className="text-sm text-sky-300">Tier {result.tier}</div>
...
function QuestionView({
  ...
  return (
    <div className="space-y-1.5">
      ...
      {question.help && <div className="text-xs text-white/45">{question.help}</div>}
