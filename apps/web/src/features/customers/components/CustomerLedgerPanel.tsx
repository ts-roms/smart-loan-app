 ...
 * activity (disbursements / payments / penalty waivers) and cooperative
 * activity (contributions, savings deposits / withdrawals) into a
 * unified chronological stream plus a section-wise summary.
 *
 * Filters: scope chips (All / Loans / Coop) + date range. CSV export
 * downloads the same view as a flat file (headers + comment block carry
 * the summary, so the file is self-contained).
 */
export function CustomerLedgerPanel({ idOrNumber }: { idOrNumber: string }) {
  ...
  const onExportCsv = async () => {
    setDownloading(true);
    try {
      await downloadCustomerLedgerCsv(idOrNumber, {
        scope,
        from: from || undefined,
        to: to || undefined,
      });
  ...
  const onExportPdf = async () => {
    ...
    try {
      ...
      await downloadPdf(
        `/customers/${idOrNumber}/ledger.pdf?${params.toString()}`,
        `statement-${idOrNumber}.pdf`,
      );
    } catch (err) {
  ...
  const onEmailCustomer = async () => {
    setEmailing(true);
    try {
      const result = await emailCustomerStatement(idOrNumber);
      ...
      <CardHeader className="flex flex-row items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-fg-muted mt-1">
            Unified statement of account — loans, contributions, and savings.
          </p>
        </div>
...
function SummaryStats({
  ...
  summary: import("@loan/shared-types").CustomerLedgerSummary;
}) {
  return (
