...
export function AccountingDashboardPage() {
  ...
  if (trial.isLoading || incomeStmt.isLoading || balance.isLoading || aging.isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard label="Cash" value={formatMoney(cash)} icon={CircleDollarSign} accent="emerald" />
        <StatCard label="Loans receivable" value={formatMoney(receivable)} icon={Receipt} accent="sky" />
        <StatCard label="Net income YTD" value={formatMoney(netIncome)} icon={TrendingUp} accent="amber" />
        <StatCard label="Delinquent" value={formatMoney(delinquent)} icon={Landmark} accent="rose" />
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            <ReportLink to="/accounting/trial-balance" title="Trial balance" subtitle={`${trial.data?.inBalance ? 'In balance' : 'Out of balance'}`} />
            <ReportLink to="/accounting/income-statement" title="Income statement" subtitle={`Net: ${formatMoney(netIncome)}`} />
            <ReportLink to="/accounting/balance-sheet" title="Balance sheet" subtitle={`Assets: ${formatMoney(balance.data?.assets.total ?? 0)}`} />
            <ReportLink to="/accounting/portfolio" title="Loan portfolio" subtitle={`${aging.data?.rows.length ?? 0} active`} />
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent>
            <ul className="divide-y divide-white/5 text-sm">
              {(['CURRENT', 'D_1_30', 'D_31_60', 'D_61_90', 'D_90_PLUS'] as const).map((b) => (
                <li key={b} className="flex items-center justify-between py-2">
                  <span className="text-white/70">{labelFor(b)}</span>
                  <span className="font-mono">{formatMoney(aging.data?.totals[b] ?? 0)}</span>
                </li>
              ))}
        ...
        <Card>
          ...
          <CardContent>
            <ul className="space-y-2 text-sm">
              <li>
                <Link className="text-sky-300 hover:underline" to="/accounting/accounts">
                  Chart of accounts →
                </Link>
              </li>
              <li>
                <Link className="text-sky-300 hover:underline" to="/accounting/journal">
                  Journal entries →
                </Link>
              </li>
              <li>
                <Link className="text-sky-300 hover:underline" to="/accounting/portfolio">
                  Loan portfolio aging →
                </Link>
              </li>
              <li>
                <Link className="text-sky-300 hover:underline" to="/accounting/periods">
                  Periods & accrual →
                </Link>
              </li>
            </ul>
...
function labelFor(bucket: string): string {
  switch (bucket) {
    ...
    case 'D_1_30': return '1–30 days overdue';
    case 'D_31_60': return '31–60 days overdue';
    case 'D_61_90': return '61–90 days overdue';
    case 'D_90_PLUS': return '90+ days overdue';
...
function ReportLink({ to, title, subtitle }: { to: string; title: string; subtitle: string }) {
  return (
    <Link
      to={to}
