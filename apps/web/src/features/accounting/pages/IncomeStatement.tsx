...
import { Card, CardContent, CardHeader, CardTitle, DatePicker, SkeletonCard } from '@loan/ui';
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <DatePicker value={from} onChange={setFrom} max={to} className="h-9 w-44" />
          <label className="text-white/55">To</label>
          <DatePicker value={to} onChange={setTo} min={from} className="h-9 w-44" />
        </div>
      </CardHeader>
      <CardContent>
        {report.isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Section title="Income" rows={report.data.income.rows} total={report.data.income.total} positive />
            <Section title="Expenses" rows={report.data.expense.rows} total={report.data.expense.total} />
            <div className="md:col-span-2 border-t border-white/10 pt-3 flex items-center justify-between">
              <div className="text-sm uppercase tracking-wider text-white/55">Net income</div>
              <div
                className={`text-xl font-semibold font-mono ${
                  report.data.netIncome >= 0 ? 'text-emerald-300' : 'text-rose-300'
                }`}
              >
    ...
    <div>
      ...
      <ul className="divide-y divide-white/5 text-sm">
        ...
        <li className="flex justify-between py-2 border-t border-white/10 font-semibold mt-1">
          ...
          <span className={`font-mono ${positive ? 'text-emerald-300' : ''}`}>{formatMoney(total)}</span>
        </li>
      </ul>
    </div>
  );
