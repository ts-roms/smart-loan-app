      ...
      <Card>
        ...
        <CardContent>
          ...
          {roles.isLoading ? (
            ...
            <p className="text-sm text-fg-muted">No roles configured.</p>
          ) : (
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div className="rounded-md border border-default">
            <div className="px-3 py-2 text-xs uppercase tracking-wider text-fg-subtle border-b border-default flex items-center justify-between">
              <span>Inherits from ({parents.size})</span>
              {parents.size > 0 && (
                <button
                  type="button"
                  onClick={() => setParents(new Set())}
                  className="text-info hover:underline normal-case tracking-normal"
                >
                  Clear
                </button>
              )}
            ...
            <div className="px-3 py-1.5 text-[10px] text-fg-subtle border-t border-default">
          ...
          <div className="rounded-md border border-default">
            ...
            <div className="divide-y divide-default max-h-80 overflow-y-auto">
              {grouped.map(([category, perms]) => {
                const allOn = perms.every((p) => selected.has(p.key));
                const someOn = perms.some((p) => selected.has(p.key));
                return (
                  <div key={category} className="p-2">
                    <label className="flex items-center gap-2 text-xs uppercase tracking-wider text-fg-muted mb-1">
                      <input
                        type="checkbox"
                        checked={allOn}
                        ref={(el) => {
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3 text-sm">
          <p className="text-fg">
            ...
            <span className="font-mono text-fg">{impact.role.key}</span>. Active
          ...
          <ul className="space-y-1.5 rounded-md border border-warning/30 bg-warning/[0.04] p-3">
            {atRisk.map((r) => (
              ...
              >
                <span>
                  <span className="font-mono text-warning">{r.key}</span>
                  <span className="ml-2 text-fg">{r.label}</span>
                </span>
